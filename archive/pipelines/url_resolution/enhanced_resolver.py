...
class EnhancedURLResolver:
    ...
    def __init__(
        ...
        dead_links_file: str = "dead_links_cache.txt",
    ):
        ...
        self.dead_links_file = dead_links_file

        # Load dead links into memory set for O(1) lookups
        self.dead_links = set()
        if os.path.exists(self.dead_links_file):
            try:
                with open(self.dead_links_file, 'r') as f:
                    self.dead_links = set(line.strip() for line in f if line.strip())
                print(f"Loaded {len(self.dead_links)} known dead links.")
            except Exception as e:
                print(f"Error loading dead links file: {e}")
        ...
        http_adapter = HTTPAdapter(pool_connections=60, pool_maxsize=200)
        https_adapter = HTTPAdapter(pool_connections=60, pool_maxsize=200)

        self.session.mount("http://", http_adapter)
        self.session.mount("https://", https_adapter)
        
        # Reduce default timeout
        self.timeout = max(3, min(timeout, 15))  # Cap at 15s, min 3s
    ...
    def resolve_single_url(self, url: str) -> Dict:
        ...
        """
        # Check dead links cache
        if url in self.dead_links:
            self.logger.debug(f"Skipping known dead link: {url}")
            return {
                "original_url": url,
                "resolved_url": url,
                "status_code": 0,
        ...
        try:
            ...
            try:
                ...
                # Handle the same redirect logic as above (simplified)
                # Check for HTTP redirects
                if response.status_code in [301, 302, 303, 307, 308]:
                    location = response.headers.get("location")
                    if location:
                         # We'll accept it worked but stop here for simplicity in error handling block
                         # Or just consider it resolved if it gave a 200
                         pass

                result = {
                    "original_url": url,
                    "resolved_url": url, # Keep original if we just verified it exists
                    "status_code": response.status_code,
                    "redirect_count": 0,
                    "success": True,
                    "error": "SSL bypassed",
                ...
                # Add to dead links if it failed again
                self._add_to_dead_links(url)
                result = {
                    "original_url": url,
                    "resolved_url": url,
                    "status_code": None,
    ...
    def _add_to_dead_links(self, url: str):
        """Add URL to dead links cache."""
        if url not in self.dead_links:
            self.dead_links.add(url)
            try:
                with open(self.dead_links_file, 'a') as f:
                    f.write(f"{url}\n")
            except Exception as e:
                self.logger.warning(f"Error writing to dead links file: {e}")

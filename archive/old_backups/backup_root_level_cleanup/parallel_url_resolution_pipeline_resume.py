#!/usr/bin/env python3
"""
Parallel URL Resolution Pipeline (Resume Capable)
================================================
Loads raw URLs from browser and Facebook extraction, resolves them in parallel,
and saves results incrementally to disk. Can resume from where it left off.
"""
...
class ParallelURLResolutionPipelineResume:
    def __init__(
        self, base_dir: str = "../data", batch_size: int = 1000, max_workers: int = 20
    ):
        ...
        self.logger.info(
            f"Initialized ParallelURLResolutionPipelineResume (batch_size={batch_size}, max_workers={max_workers})"
        )

    def setup_logging(self):
        log_file = (
            self.output_dir
            / f"parallel_pipeline_resume_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
        logging.basicConfig(
            level=logging.INFO,
    ...
    def get_next_batch_number(self, source: str) -> int:
        """Get the next available batch number for the source."""
    ...
    def resolve_and_save_batches_resume(self, urls: List[str], source: str):
        total = len(urls)
        successful_resolutions = 0
        failed_resolutions = 0

        # Get the next batch number to resume from
        next_batch_num = self.get_next_batch_number(source)
        self.logger.info(
            f"Resuming {source} URL resolution from batch {next_batch_num}"
        )

        self.logger.info(
            f"Starting parallel resolution of {total} {source} URLs with {self.max_workers} workers..."
        )
    ...
    def run(self):
        self.logger.info(
            "=== Starting Parallel URL Resolution Pipeline (Resume Capable) ==="
        )
        ...
        for source in ["browser", "facebook"]:
            ...
            if not df.empty:
                ...
                self.resolve_and_save_batches_resume(urls, source)
                total_processed += len(urls)

        self.logger.info(
            "=== Parallel URL Resolution Pipeline (Resume Capable) Finished ==="
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parallel URL Resolution Pipeline (Resume Capable)"
    )
    ...
    pipeline = ParallelURLResolutionPipelineResume(
        base_dir=args.base_dir, batch_size=args.batch_size, max_workers=args.max_workers
    )
    pipeline.run()

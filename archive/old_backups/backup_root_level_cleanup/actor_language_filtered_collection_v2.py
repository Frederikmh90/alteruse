...
def collect_posts_from_language_actors(db, actor_usernames: List[str], platform: str):
    ...
    print(
        f"  🔄 Processing {len(actor_batches)} batches of {ACTOR_BATCH_SIZE} actors each..."
    )
    ...
    with ThreadPoolExecutor(max_workers=ACTOR_BATCH_WORKERS) as executor:
        futures = [
            executor.submit(process_actor_batch, batch) for batch in actor_batches
        ]
        ...
        for i, future in enumerate(tqdm(futures, desc="  Collecting posts")):
            ...
            if (i + 1) % 3 == 0 or i == len(futures) - 1:
                ...
                print(
                    f"    Batch {i + 1}/{len(futures)}: +{len(batch_post_ids)} posts (total: {len(all_post_ids):,}, avg: {avg_per_batch:.0f}/batch)"
                )
    ...
    if (
        len(all_post_ids) > MAX_POSTS_PER_ACTOR
        if MAX_POSTS_PER_ACTOR
        else len(all_post_ids)
    ):
        target_posts = int(

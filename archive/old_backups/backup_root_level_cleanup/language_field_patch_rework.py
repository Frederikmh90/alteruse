...
# =========================
PLATFORMS = "facebook"  # All platforms
LANG_FILTER = {"da", "de", "sv"}  # Danish, German, Swedish ONLY
ACCOUNT_PERCENTAGE = 10  # 10% of actors per platform #100%
POST_PERCENTAGE = 40  # 5% of posts per actor # 40 %
MIN_POSTS_PER_ACTOR = 30  # Minimum posts per actor
MAX_POSTS_PER_ACTOR = 0  # Maximum posts per actor
POSTS_PER_ACTOR_SOFT = 0  # e.g. 2000
MAX_ACTORS_PER_PLATFORM = 10_000_000
BATCH_FETCH_SIZE = 600
FETCH_WORKERS = 8
CHUNK_SAVE_SIZE = 100_000

OUTPUT_DIR = "./data/climate_obstruction_sampling_teest"
RANDOM_SEED = 42
...
POST_PROJECTION_MIN = {
    ...
    # dropped heavy field: platformId
    # telegram specifics
    "from_username": 1,
    ...
    "body": 1,
    "url": 1,
    # generic external link fields (e.g., google/web)
    "link": 1,
    "displayLink": 1,
    # dropped heavy field: pagemap
    # link helpers
    "post_url": 1,
    "postUrl": 1,
    ...
    "author": 1,
    "entities": 1,
    # dropped heavy field: public_metrics
    "attachments": 1,
    "referenced_tweets": 1,
    # dropped heavy fields: type_id, name, statistics, user_id
}
...
def _first_url_in_text(text: str | None) -> str | None:
...
def _extract_external_link(post: Dict[str, Any]) -> str | None:
    # Direct link fields (e.g., web/google results)
    direct = post.get("link")
    if isinstance(direct, str) and direct.startswith("http"):
        return direct

    # Telegram: media.webpage.url
    media = post.get("media")
    if isinstance(media, dict):
        webpage = media.get("webpage")
        if isinstance(webpage, dict):
            wurl = webpage.get("url")
            if isinstance(wurl, str) and wurl.startswith("http"):
                return wurl

    # Twitter: entities.urls[].expanded_url | unwound_url | url
    ...
    # Reddit: external link in 'url' (exclude reddit domains)
    rurl = post.get("url")
    if isinstance(rurl, str) and rurl.startswith("http"):
        try:
            host = urlparse(rurl).netloc.lower()
            if not (host.endswith("reddit.com") or host.endswith("redd.it")):
                return rurl
        except ValueError:
            # Malformed IPv6/URL; treat as external link and let domain parsing handle None
            return rurl

    # VKontakte: attachments[].type == 'link'
    ...
    # Fallback: first URL in textual fields
    text_val = (
        post.get("text")
        or post.get("message")
        or post.get("caption")
    ...
    return _first_url_in_text(text_val)


def _domain_from_link_or_display(post: Dict[str, Any], link: str | None) -> str | None:
    if isinstance(link, str) and link:
        try:
            host = urlparse(link).netloc.lower()
            return host[4:] if host.startswith("www.") else host
        except ValueError:
            # Malformed IPv6/URL in link
            return None
    disp = post.get("displayLink")
    if isinstance(disp, str) and disp:
        disp_l = disp.lower()
        return disp_l[4:] if disp_l.startswith("www.") else disp_l
    return None
...
def _rows_from_posts(
    ...
    for p in posts:
        ...
        external_link_val = _extract_external_link(p)
        domain_val = _domain_from_link_or_display(p, external_link_val)
...
def _fetch_post_docs_by_ids(db, post_ids: List[Any]):
    ...
    return list(
        db.post.find(
            {"_id": {"$in": post_ids}}, POST_PROJECTION_MIN, max_time_ms=120000
        )
    )
...
def sample_one_platform(db, platform: str, lang_filter: Set[str]):
    ...
    print(f"🧮 {platform}: Processing {len(batches)} batches (size≈{BATCH_FETCH_SIZE})")
    ...
    with ThreadPoolExecutor(max_workers=FETCH_WORKERS) as ex:
        ...
        for fut in tqdm(
            ...
            try:
                ...
                print(f"⚠️  {platform} batch failed: {e}")
                continue

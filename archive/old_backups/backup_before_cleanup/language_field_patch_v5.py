...
POST_PERCENTAGE = 40  # 5% of posts per actor
...
MAX_ACTORS_PER_PLATFORM = 100_000_000
BATCH_FETCH_SIZE = 1000
FETCH_WORKERS = 10
CHUNK_SAVE_SIZE = 100_000

OUTPUT_DIR = "./data/climate_obstruction_sampling_v2"
RANDOM_SEED = 42
...
def _rows_from_posts(
    ...
    for p in posts:
        ...
        # Prefer Spread getters for link + domain, then fall back to robust extractors
        external_link_val = _safe_get(Spread._get_message_link, m, p)
        if isinstance(external_link_val, Exception) or not external_link_val:
            external_link_val = _extract_external_link(p)

        domain_val = _safe_get(Spread._get_message_link_domain, m, p)
        if isinstance(domain_val, Exception) or not domain_val:

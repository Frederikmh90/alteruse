#!/usr/bin/env python3
"""
Parallel URL Resolution Pipeline (Resume Capable - Fixed)
========================================================
Loads raw URLs from browser and Facebook extraction, resolves them in parallel,
and saves results incrementally to disk. Properly resumes from where it left off.
"""
...
class ParallelURLResolutionPipelineResumeFixed:
    def __init__(
        ...
        self.logger.info(
            f"Initialized ParallelURLResolutionPipelineResumeFixed (batch_size={batch_size}, max_workers={max_workers})"
        )

    def setup_logging(self):
        log_file = (
            self.output_dir
            / f"parallel_pipeline_resume_fixed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
        logging.basicConfig(
            level=logging.INFO,
    ...
    def resolve_and_save_batches_resume_fixed(self, urls: List[str], source: str):
        total_urls = len(urls)
    ...
    def run(self):
        self.logger.info(
            "=== Starting Parallel URL Resolution Pipeline (Resume Capable - Fixed) ==="
        )
        ...
        for source in ["browser", "facebook"]:
            ...
            if not df.empty:
                ...
                self.resolve_and_save_batches_resume_fixed(urls, source)
                total_processed += len(urls)

        self.logger.info(
            "=== Parallel URL Resolution Pipeline (Resume Capable - Fixed) Finished ==="
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parallel URL Resolution Pipeline (Resume Capable - Fixed)"
    )
    ...
    pipeline = ParallelURLResolutionPipelineResumeFixed(

...
from .news_analysis import (
    analyze_facebook_directory,
    save_analysis_results as save_analysis,
)
...
def _get_facebook_dataframes_from_analysis(
    folder_path: str,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Compatibility wrapper around analyze_facebook_directory.

    The consolidated news module currently returns a dict placeholder.
    Older code expects a tuple (df_urls, df_pages). This function adapts
    whatever is returned into two DataFrames.
    """
    try:
        result = analyze_facebook_directory(folder_path)

        # Already the expected (df_urls, df_pages)
        if isinstance(result, tuple) and len(result) == 2:
            df_urls, df_pages = result
            # Ensure DataFrame types
            df_urls = (
                df_urls if isinstance(df_urls, pd.DataFrame) else pd.DataFrame(df_urls)
            )
            df_pages = (
                df_pages
                if isinstance(df_pages, pd.DataFrame)
                else pd.DataFrame(df_pages)
            )
            return df_urls, df_pages

        # Dict placeholder: try common keys; otherwise return empty frames
        if isinstance(result, dict):
            urls_candidate = (
                result.get("df_urls")
                or result.get("urls")
                or result.get("url_records")
                or []
            )
            pages_candidate = result.get("df_pages") or result.get("pages") or []

            df_urls = (
                urls_candidate
                if isinstance(urls_candidate, pd.DataFrame)
                else pd.DataFrame(urls_candidate)
            )
            df_pages = (
                pages_candidate
                if isinstance(pages_candidate, pd.DataFrame)
                else pd.DataFrame(pages_candidate)
            )
            return df_urls, df_pages

    except Exception as e:
        print(
            f"Error adapting analyze_facebook_directory return for {folder_path}: {e}"
        )
...
def analyze_facebook_json_data(folder_path: str) -> Dict[str, Any]:
    ...
    # Use existing analyze_facebook_directory to get data (with compatibility wrapper)
    df_urls, df_pages = _get_facebook_dataframes_from_analysis(folder_path)
...
def analyze_account_activity(folder_path: str) -> Optional[Dict[str, Any]]:
    ...
    # Get URL and pages data using existing function (with compatibility wrapper)
    df_urls, df_pages = _get_facebook_dataframes_from_analysis(folder_path)
    ...
    for folder in facebook_folders:
        try:
            ...
            if metrics:  # Only add if we got valid metrics
                ...
                # Get URLs and pages data for this account (with compatibility wrapper)
                df_urls, df_pages = _get_facebook_dataframes_from_analysis(folder)
...
# /Users/Codebase/projects/alteruse/data/Kantar_02_10
def main():
    # Define paths
    kantar_dir = "/Users/Codebase/projects/alteruse/data/Kantar_02_10/Facebook/"
    output_dir = (
        "/Users/Codebase/projects/alteruse/data/Analyzed_Kantar_251002/Facebook"
    )

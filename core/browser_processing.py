...
def process_safari_json(json_path: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # Filter to only include records that have a URL for news analysis
        if "url" not in df.columns:
            print(f"No 'url' column found in {json_path}")
            return None

        # Remove records without URLs as they can't be analyzed for news sources
        initial_count = len(df)
        df = df[df["url"].notna() & (df["url"] != "")]
        if len(df) < initial_count:
            print(f"Filtered out {initial_count - len(df)} records without URLs")
...
def process_chrome_json(json_path: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            for key in data:
                if isinstance(data[key], list) and len(data[key]) > 0:
                    if isinstance(data[key][0], dict) and "url" in data[key][0]:
                        data = data[key]
                        break
        ...
        # Handle different timestamp column names (include Chrome JSON 'lastVisitTime')
        time_columns = [
            "visitTime",
            "time",
            "visit_time",
...
def main():
    # Define paths
    base_dir = "/Users/Codebase/projects/alteruse/data/Kantar_02_10/Browser/"
    output_dir = "/Users/Codebase/projects/alteruse/data/Analyzed_Kantar_251002/Browser"
